# This file is derived from MLIP-2 (mlippy).
#
# Copyright (c) 2023 Alexander Shapeev (Skoltech)
#
# Modifications by J H Jung, so that the written cfg file
# could be read by the readcfg function:
# - Changed CFG coordinate formatting to 6 decimal places.
# - Changed force formatting.
# - Changed energy formatting to match VASP OUTCAR.
# These changes are indicated ("change beg/end")
#
# See LICENSE.mlippy for the original BSD 2-Clause license.
...
def savecfg(f, cfg, desc=None):
    ...
    for i in range(size):
        if cfg.forces is not None:
            # change beg: alignment and 6 decimal digits like vasp OUTCAR force
            # print('         %4d %4d %13f%13f%13f %11.8e %11.8e %11.8e' %
            print('         %4d %4d  %13.6f %13.6f %13.6f  %11.6f %11.6f %11.6f' %
                  # change end
