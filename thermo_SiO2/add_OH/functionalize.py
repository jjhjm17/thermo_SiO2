...
assert ValueError("Error: When we use this code, some OH are too far from Si. Use Tom's code")
...
sym_func = ['Si', 'O', 'H', 'Al']  # order for input and output

if args.format == 'lammps-dump-text':
    # atoms = read(dump_file, format='lammps-dump-text', specorder=symbols)
    atoms = read(dump_file, format='lammps-dump-text', specorder=sym_func)
